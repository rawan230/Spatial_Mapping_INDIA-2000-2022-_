...
## Audit of record (2026-09-24/25) — read before citing any number

A full end-to-end audit recalculated every step from raw data and re-ran every model. **It supersedes
older headline claims in this file and in the root documents.** Start at
`results/FULL_METHODOLOGY_AUDIT.md`, and take paper numbers **only** from
`results/FINAL_MANUSCRIPT_NUMBERS.md`. The root documents carry an "Audit update (2026-09-25)" block
that lists their superseded statements; pre-update copies are in
`_Archive_Unwanted_2026-09-25/pre_audit_document_snapshots/`.

Essentials:

- **Reproduction:** all historical numbers reproduce bit-exactly. The v1 artefacts are kept as
  "historical"; the corrected **v2** feature table is `results/recalculated/FEATURE_TABLE_v2.parquet`
  (55 features, corrected label).
- **Defects fixed in v2:**
  - Half-pixel fire rasterization (see the conventions section).
  - Degenerate climate/LST/NDVI "anomaly-mean" features, replaced by 2001–2020 climatological levels.
  - Mann–Kendall on seasonal/autocorrelated series, replaced by Seasonal Kendall + Sen + BH-FDR.
  - In-window 2020 land-cover fractions, replaced by 2001.
  - Moran's I computed over filled non-India cells.
  - B3 terminal-label leakage.
- **Evaluation population:** forest pixels (`forest_frac_2001 > 0`) are primary, because forest
  fraction alone gives AUC 0.91 over all pixels. RF v2: 0.975 all / 0.897 forest.
- **CDR-PINO:** under one protocol with 3 seeds, **no physics configuration beats the same network
  without physics**. RF on CDR-PINO's own cells and covariates beats it on every track (0.980 / 0.974
  / 0.959 vs 0.939 / 0.719 / 0.570 on A/B1/B2). On unseen years (B3), 0.893 is **below**
  covariate-free persistence baselines (0.908 / 0.930). The trained field is static in time and does
  not satisfy its PDE.
...
Two root folders are **not** git repos and are excluded by the root `.gitignore` (`*/`):

- `results/` — the 2026-09-24/25 audit: recalculations, v2 feature table, all model runs and
  deliverables. `results/code/` holds every audit script.
- `_Archive_Unwanted_2026-09-25/` — files moved out of the root: pre-audit document snapshots,
  superseded duplicates, presentation work, early exploration, and regenerable audit scratch (~39 GB,
  safe to delete). See its `README.md`.
...
1. **Fire point extraction** (`FOREST_FIRE_POINTS_EXTRACTION(INDIA).ipynb`, generated by
   ...
   `Biswas2025_AnnualFireCount_Comparison.csv`. **2026-09-16**: the r=0.915 figure is
   computed across all of India's land cover, not forest only — unlike Biswas et al.'s
   own forest-scoped Fig. 7d, causing a 6–10× magnitude gap. A forest-masked
   re-derivation from the raw MCD64A1.061 GeoTIFFs (`Annual_BurnedArea_ForestVsAll.csv`,
   `BurnedArea_ForestMasked_vs_Biswas_Difference.csv`) closes most of it: 9,392–51,455
   km²/year vs. Biswas et al.'s ~2,100–17,200 km²/year, r=0.9044, matching min/max years
   exactly. A residual ~4× gap remains, disclosed rather than forced to close. The
   forest-masked series also correlates more tightly against this step's own
   forest-fire-point counts than the all-land-cover series did (Pearson r=0.9345 vs.
   0.9149, Spearman ρ=0.7846, n=23).
2. **NDVI features** (`NDVI_ANALYSIS_WITH_FFP.ipynb`, generated by `build_ndvi_notebook.py`)
   ...
   **NDVI grid** (3641×3504 px, EPSG:4326, **1/120° ≈ 0.93 km**, not 0.01°) that Steps 3,
   4, 5, and 6 all reproject onto. *Audit 2026-09-25:*
   - F3 (anomaly mean) is degenerate: it equals the residue of the 26 out-of-baseline months.
   - F5 (residual mean) is identically about 0.
   - F6 (MK on the MA-smoothed trend, lag-1 autocorrelation 0.975) is an invalid test; replaced by
     Seasonal Kendall.
   - Moran's I 0.8322 was computed with 67% filled non-India cells; India-only I = 0.9456.
   - 2007-03/04 have no pixel-reliability layer; they were used unfiltered.
...
6. **Integrated alignment** (`Step6_Integrated_FireRisk_Analysis.ipynb`, was
   ...
   (4,161,009 in-India pixels × **61 columns / 57 features** after dropping
   `lon`/`lat`/`fire_count`/label, verified 2026-09-25; full 15/15 Biswas et al.
   predictor-group parity, but in v1 the climate/LST variables enter only as degenerate
   anomaly-means and MK trends, not as the levels Biswas used). *Audit 2026-09-25:* the
   22 land-cover fractions still come from the **2020** map, which is inside the label
   window. The v2 table uses 2001 fractions and climatological levels.
...
7. **Susceptibility model** (`Step7_FireRisk_Susceptibility_Model.ipynb`, was
   ...
   **RF ROC-AUC 0.9704**, AP 0.7011. **2026-08-23: MaxEnt got the same validated-tuning
   ...
   GroupKFold anchored at 0°): RF 0.9498±0.0035, MaxEnt 0.9465±0.0054. *Audit 2026-09-25:*
   these are **not** the same folds, grid, label or features as CDR-PINN's Track B1. The
   fair same-cells comparison is in `results/` (RF 0.974 vs CDR 0.719 on B1). v2
   results: RF 0.975 all / 0.897 forest; MaxEnt uses 150k training rows (sensitivity
   50k–500k in `results/final/SENSITIVITY_RESULTS.csv`). Kept as a classical-ML baseline, not a PINN
...
8. **PINN comparison** (`Step8_PINN_FireRisk_Model.ipynb` + `Step8b_PINN_Seed_Robustness_
   ...
   **Audit 2026-09-25 supersedes the interpretation above** (see `results/`). All
   historical numbers reproduce bit-exactly, but:
   - Track A and B1/B2/B3 were separately trained models under different protocols.
   - Under one validated protocol with 3 seeds: no physics 0.945, diffusion 0.924,
     diff+adv 0.939, full 0.939 (Track A). B1 0.724 vs 0.719, B2 0.614 vs 0.570, B3 0.904
     vs 0.893 (no physics vs full). **No physics term improves accuracy.**
   - "Temporal generalization is the one clear advantage" is **withdrawn**: B3 is below
     covariate-free persistence nulls (0.908 / 0.930).
   - "Elevation dominance" is specific to CDR-PINN's 7 covariates; terrain is redundant in
     the full predictor set.
   - The trained field is static in time and violates its PDE (residual 74–616× |du/dt|).
   - The historical ablation's full-CDR checkpoint was overwritten by the Track A checkpoint.
   - Reaction term: "Fisher-KPP" is a misnomer, because the reaction acts on the logit.
...
- **Fire-point rasterization pattern (CORRECTED 2026-09-25)**: use the affine transform
  directly, not a spatial join, and compute the **containing pixel**:
  `col = floor((lon-c)/a)`, `row = floor((lat-f)/e)`.
  - Here `c, f` are the top-left pixel **edge** coordinates, not a pixel centre.
  - The historical rule `round((lon-c)/a)` / `round((lat-f)/e)` rounds against the edge and put
    **74.9% of the 541,545 points in a neighbouring pixel**. The historical F10 raster and the v1
    `fire_ever` label carry this shift.
  - The corrected label is `y_all` in `results/recalculated/FEATURE_TABLE_v2.parquet`.
  - `round` is only correct when the reference coordinates are pixel **centres**, e.g. the
    lat/lon arrays of the LCCS NetCDF used in Step 1's forest filter.
...
| `firerisk-anaconda3` | 3.12.7 | `C:\Users\Admin\anaconda3\python.exe` (base anaconda3; **torch 2.12.1 is CPU-only**) | Steps 4, 5, 7 notebooks; audit scripts |
| `cdr_pinn_env` (no Jupyter kernel) | 3.11.15 | `C:\Users\Admin\anaconda3\envs\cdr_pinn_env\python.exe` (torch 2.11.0+cu128) | **All CDR-PINO training** (`Physics_Informed_FireRisk_Model/cdr_pinn/*.py`, `results/code/cdr_*.py`); the only env with CUDA torch |

Corrections verified 2026-09-25: the Step 6 notebook's kernelspec is `wildfire_env` (not
`firerisk-anaconda3`), and CDR-PINO scripts must run in `cdr_pinn_env`.
